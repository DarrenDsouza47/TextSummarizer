...
class Model_Evaluation:
    ...
    def calculate_metric_on_test_ds(self,dataset,metric,model,tokenizer,device,batch_size=16,column_text="article",column_summary="highlights"):
        ...
        for article_batch,target_batch in tqdm(zip(article_batches,target_batches),total=len(article_batches)):
            inputs=self.tokenizer(article_batch,max_length=1024,truncation=True,padding="max_length",return_tensors="pt")
    ...
    def evaluate(self):
        ...
        score=self.calculate_metric_on_test_ds(samsum_dataset_pt["test"][0:10],metric,model,tokenizer,batch_size=2,device=device,column_text="dialogue",column_summary="summary")

        rouge_dict=dict((rn,score[rn].mid.fmeasure)for rn in metric_name)
